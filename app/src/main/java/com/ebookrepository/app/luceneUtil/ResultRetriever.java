...
public final class ResultRetriever {
	
	private static int maxHits = 100;
	private static final Version matchVersion = Version.LUCENE_4_9;
	private static Analyzer analyzer = new StandardAnalyzer(matchVersion);
	 ...
	 * 
	 * @param query - upit po kojem ce biti trazeni dokumenti
	 * @param analyzeQuery - da li je potrebno analizirati upit.<p>Za vrednost:<br/> - <i>true</i> -> upit ce biti analiziran postojecim analyzer-om<br> - <i>false</i> -> upit nece biti analiziran<p><b>Napomena:</b> podrazumevani analyzer je {@link StandardAnalyzer}. Za promenu analyzer-a koristiti metodu {@link #setAnalyzer(Analyzer)} 
	 * @return Lista dokumenata koji odgovaraju (analiziranom) upitu.<br> - praznu listu ukoliko ni jedan dokument ne odgovara upitu<br> - <b>null</b> ukoliko je prosledjeni upit null
	 * @throws IllegalArgumentException ako se u direktorijumu ne nalaze indeksi ili je direktorijum zakljucan
	...
	public static void main(String[] args) throws Exception{
		//primer kako funkcionise QueryParser
		String text = "First.Second or and";
		Query tq = new TermQuery(new Term("ime", text));
		System.out.println(tq);
		System.out.println(new QueryParser(matchVersion, "", analyzer).parse(tq.toString()));
		
		//primer kako izbor analyzera utice na konacan upit
		System.out.println();
		Query upit1 = new QueryParser(matchVersion, "name", new SimpleAnalyzer(matchVersion)).parse(text);
		Query upit2 = new QueryParser(matchVersion, "name", analyzer).parse(text);
